...
import torch
class TimeCartoonDataset(Dataset):
        ...
        def __init__(self, root, path_file, transform=None):
                ...
                self.max_frame_num = np.max([int(p.split('/')[-1].split('.')[0]) for p in self.paths])
                self.max_shot_num = np.max([int(p.split('/')[-2].split('_')[1]) for p in self.paths])
